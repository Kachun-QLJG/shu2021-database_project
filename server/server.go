...
type User struct { //客户表
	...
	Name          string `gorm:"type:varchar(100)"`                                 //姓名
...
type TypeOverview struct { //工种总览表
	ProjectNumber string `gorm:"type:varchar(8);not null"`   //维修项目编号
	ProjectName   string `gorm:"type:varchar(100);not null"` //维修项目名称
	WorkHour      string `gorm:"type:int(3);not null"`       //工时
	Type          string `gorm:"type:varchar(10);not null"`  //工种
}

type PartsOverview struct { //零件总览表
	PartsNumber string  `gorm:"type:varchar(8);not null"`  //零件编号
...
type Vehicle struct { //车辆表
	Number        string `gorm:"type:varchar(17);not null;unique_index;primary_key"` //车架号
	LicenseNumber string `gorm:"type:varchar(10);not null"`                          //最后一次访问的时间戳（精确到秒）
	UserLink      User   `gorm:"ForeignKey:UserID"`                                  //外键，用户表
	...
	Type          string `gorm:"type:varchar(10);not null"`                          //车型
}
...
type Attorney struct { //委托书表
	...
	UserLink          User     `gorm:"ForeignKey:UserID"`                                  //外键，用户表
	UserID            string   `gorm:"type:varchar(8);not null"`                           //客户编号
	VehicleLink       Vehicle  `gorm:"ForeignKey:VehicleNumber"`                           //外键，车辆表
	...
	SalesmanLink      Salesman `gorm:"ForeignKey:SalesmanID"`                              //外键，业务员表
...
type Arrangement struct { //派工单表
	AttorneyLink    Attorney      `gorm:"ForeignKey:OrderNumber"`     //外键，委托书表
	OrderNumber     string        `gorm:"type:varchar(11);not null"`  //订单号
	ProjectNumber   string        `gorm:"type:varchar(8);not null"`   //维修项目编号
	RepairmanLink   Repairman     `gorm:"ForeignKey:RepairmanNumber"` //外键，委托书表
	RepairmanNumber string        `gorm:"type:varchar(8);not null"`   //订单号
	PartsLink       PartsOverview `gorm:"ForeignKey:PartsNumber"`     //外键，零件总览表
...
func main() {
	...
	database.DB().SetConnMaxLifetime(time.Hour * 4) //设置超时时间
	// 创建一个默认的路由引擎
	r := gin.Default()
	...
	r.LoadHTMLFiles("./html/login.html", "./html/logout.html", "./html/register.html", "./html/error.html",
		"./html/success.html", "./html/index.html") //加载html模板
